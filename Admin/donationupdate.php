                ...
                <div class="col-md-8 col-sm-8 col-lg-8 col-xlg-9">
                    ...
                    <div class="well">
                        <form action="" method="post" id="frm" name="frm" enctype="multipart/form-data">
							<input id="check" name="check" type="hidden"  value="93" />
							<input id="doid" name="doid" type="hidden"  value="<?php echo $urlid; ?>" />
							
							<div class="form-group">
								<label class="control-label mb-1">Donation Detail</label>
								 <textarea class="textarea" placeholder="Place some text here" id="dodetail" name="dodetail" ></textarea>
							</div>
							<div class="form-group">
								<label  class="control-label mb-1">Paytm Image</label>
								<input type="file"  id="paimage" name="paimage" class="form-control">
							</div>
							<div class="form-group">
								<label  class="control-label mb-1">PhonePay Image</label>
								<input type="file"  id="phimage" name="phimage" class="form-control">
							</div>
							
							<div class="from-group mb-1">
									<label class="control-label ">Status</label>
									<select name="dostatus" id="dostatus" class="form-control">
									<option value="Active">Active</option>
									<option value="Deactive">Deactive</option>
									</select>
							</div><br>
							<div>
							<center><input type="submit" class="btn btn-success" name="btn1" value="Update"> <input type="button" class="btn btn-success" name="btn2" id="btn2" value="Delete"></center>
							</div>
								<script> CKEDITOR.replace('dodetail');</script>
		...
		$.ajax(
			{
				url:"check.php",
				method:"POST",
				data:{check:92,urlid:id},
				success:function(data)
				{
					let jdata = JSON.parse(data);
					$("#dodetail").val(jdata[1]);
					$("#dostatus").val(jdata[4]);
				}
			});
			$("#btn2").click(function()
			{
				let id=$("#doid").val();
				$.post("check.php",{check:94,urlid:id},function(data)
				{
					window.location="donationview.php";
					$("#show").html(data);
				});
			});
