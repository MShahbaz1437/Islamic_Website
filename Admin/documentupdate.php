<?php $uid=$_GET["urlname"];?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Documents Update</title>
...
<body style="background-color:#eef5f9;">
                ...
                <div class="col-md-8 col-sm-8 col-lg-8 col-xlg-9">
                    ...
                    <div class="well">
                        <form action="" method="post" id="frm" name="frm" enctype="multipart/form-data">
							<input id="check" name="check" type="hidden"  value="41" />
							<input id="tid" name="tid" type="hidden"  value="<?php echo $uid; ?>" />
							
							<div class="form-group">
								<label  class="control-label mb-1">Document Name</label>
								<input type="text"  id="tdname" name="tdname" class="form-control" placeholder="Enter Document Name">
							</div>
							
							<div class="form-group">
								<label  class="control-label mb-1">Document Pdf</label>
								<input type="file"  id="tdocument" name="tdocument" class="form-control">
							</div>
	...
	$(document).ready(function()
	{
		...
		$.ajax(
		{
				url:"check.php",
				method:"POST",
				data:{check:40,urldata:id},
				success:function(data)
				{
					let jdata = JSON.parse(data);
					$("#tdname").val(jdata[1]);
					$("#tdocument").val(jdata[2]);
					$("#tstatus").val(jdata[3])
				}
		});
		$("#btn2").click(function()
		{
			var dlid = $("#tid").val();
			$.post("check.php",{check:42,urlvalue:dlid},function(data)
				{
					window.location="documentsview.php";
					$("#show").html(data);
				});
		});
