...
<body style="background-color:#eef5f9;">
                ...
                <div class="col-md-8 col-sm-8 col-lg-8 col-xlg-9">
                    ...
                    <div class="well">
                        <form action="" method="post" id="frm" name="frm" enctype="multipart/form-data">
							<input id="check" name="check" type="hidden"  value="71" />
							<input id="dtid" name="dtid" type="hidden"  value="<?php echo $urlid?>" />
							<div class="form-group">
									<label class="control-label mb-1">Dua Title</label>
									 <textarea class="textarea" placeholder="Place some text here" id="dtname" name="dtname" ></textarea>
								</div>
							<div class="from-group mb-1">
									<label class="control-label ">Status</label>
									<select name="dtstatus" id="tqstatus" class="form-control">
							...
							<div>
								...
								<input type="submit" class="btn btn-success" name="btn2" id="btn2" value="Delete"></center>
									...
									<script> CKEDITOR.replace('dtname');</script>
	...
	$(document).ready(function()
	{
		...
		$.ajax(
		{
				url:"check.php",
				method:"POST",
				data:{check:70,urldata:id},
				success:function(data)
				{
					let jdata = JSON.parse(data);
					$("#dtname").val(jdata[1]);
					$("#dtstatus").val(jdata[2])
				}
		});
		...
		{
			var dlid = $("#dtid").val();
			$.post("check.php",{check:72,urlvalue:dlid},function(data)
				{
					window.location="duatypesview.php";
					$("#show").html(data);
				});
		});
