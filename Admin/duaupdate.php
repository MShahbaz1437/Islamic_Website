...
<body style="background-color:#eef5f9;">
                ...
                <div class="col-md-8 col-sm-8 col-lg-8 col-xlg-9">
                    ...
                    <div class="well">
                        <form action="" method="post" id="frm" name="frm" enctype="multipart/form-data">
							<input id="check" name="check" type="hidden"  value="77" />
							<input id="did" name="did" type="hidden"  value="<?php echo $urlid;?>" />
							<div class="form-group">
								<label  class="control-label mb-1">Dua Type</label>
								<select type="text"  id="duatype" name="duatype" class="form-control">
								
								</select>
							</div>
							
							<div class="form-group">
								<label class="control-label mb-1">Dua Title</label><br>
								 <textarea class="textarea" placeholder="Place some text here" id="dutitle" name="dutitle" ></textarea>
							</div>
							<div class="form-group">
								<label class="control-label mb-1">Dua Detail</label>
								 <textarea class="textarea" placeholder="Place some text here" id="dudetail" name="dudetail" ></textarea>
							</div>
							<div class="form-group">
								<label  class="control-label mb-1">Dua Image</label>
								<input type="file"  id="image" name="image" class="form-control">
							</div>
							<div class="from-group mb-1">
									<label class="control-label ">Status</label>
									<select name="dustatus" id="tqstatus" class="form-control">
									<option value="Active">Active</option>
									<option value="Deactive">Deactive</option>
									</select>
							</div><br>
							<div>
								<center><input type="submit" class="btn btn-success" name="btn1" value="Update"> <input type="submit" class="btn btn-success" name="btn2" id="btn2" value="delete"></center>
							</div>
								<script>CKEDITOR.replace('dutitle'); CKEDITOR.replace('dudetail');</script>
	...
	$(document).ready(function()
	{
		...
		$.ajax(
			{
				url:"check.php",
				method:"POST",
				data:{check:76,urlid:Id},
				success:function(data)
				{
					let jdata = JSON.parse(data);
					$("#dutitle").val(jdata[1]);
					$("#dudetail").val(jdata[2]);
					$("#dustatus").val(jdata[4]);
					$("#duatype").val(jdata[5]);
				}
			});
			$("#btn2").click(function()
		{
			var Id = $("#did").val();
			$.post("check.php",{check:77, urldata:Id},function(data)
				{
					window.location="duaviewview.php";
					$("#show").html(data);
				});
		});
