                ...
                <div class="col-md-8 col-sm-8 col-lg-8 col-xlg-9">
                    ...
                    <div class="well">
                        <form action="" method="post" id="frm" name="frm" enctype="multipart/form-data">
							<input id="check" name="check" type="hidden"  value="52" />
							<input id="mid" name="mid" type="hidden"  value="<?php echo $urlid;?>" />
							<div class="form-group">
								<label  class="control-label mb-1">Menu Name</label>
								<input type="text"  id="tmname" name="tmname" class="form-control" placeholder="Enter Menu Name">
							</div>
							<div class="form-group">
								<label  class="control-label mb-1">Menu Detail</label>
								<input type="text"  id="tmdetail" name="tmdetail" class="form-control" placeholder="Enter  Detail">
							</div>
							<div class="form-group">
								<label  class="control-label mb-1">Menu Order</label>
								<input type="text"  id="tmorder" name="tmorder" class="form-control" placeholder="Enter  Order">
							</div>
							<div class="from-group mb-1">
									<label class="control-label ">Status</label>
									<select name="tmstatus" id="tqstatus" class="form-control">
									<option value="Active">Active</option>
									<option value="Deactive">Deactive</option>
									</select>
							</div><br>
							<div>
								<center><input type="submit" class="btn btn-success" name="btn1" value="Update"> <input type="submit" class="btn btn-success" id="btn2" name="btn2" value="Delete"></center>
							</div>
		...
		$.ajax(
			{
				url:"check.php",
				method:"POST",
				data:{check:51,urldata:ID},
				success:function(data)
				{
					let jdata = JSON.parse(data);
					$("#tmname").val(jdata[1]);
					$("#tmdetail").val(jdata[2]); 
					$("#tmorder").val(jdata[3]); 
					$("#tmstatus").val(jdata[4]); 
				}
			});
			$("#btn2").click(function()
			{
				var ID = $("#mid").val();
				$.post("check.php",{check:53,url:ID},function(data)
					{
