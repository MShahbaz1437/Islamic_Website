...
<body style="background-color:#eef5f9;">
                ...
                <div class="col-md-8 col-sm-8 col-lg-8 col-xlg-9">
                    <div class="well">
                        <form action="" method="post" id="frm" name="frm" enctype="multipart/form-data">
							<input id="hid" name="hid" type="hidden"  value="<?php echo $urlid ?>">
							<input id="check" name="check" type="hidden"  value="16">
							<div class="form-group">
								<label  class="control-label mb-1">Hadees Book Name</label>
								<select type="text"  id="tbname" name="tbname" class="form-control">
								</select>
							</div>
							<div class="form-group">
								<label  class="control-label mb-1">Hadees Chapter</label>
								<select type="text"  id="thcname" name="thcname" class="form-control">
								</select>
							</div>
							<div class="form-group">
								<label class="control-label mb-1">Hadees English</label><br>
								 <textarea class="textarea" placeholder="Place some text here" id="tenglish" name="tenglish" ></textarea>
							</div>
							<div class="form-group">
								<label class="control-label mb-1">Hadees Arabic</label>
								 <textarea class="textarea" placeholder="Place some text here" id="tarabic" name="tarabic" ></textarea>
							</div>
							<div class="form-group">
								<label class="control-label mb-1">Hadees Urdu</label>
								 <textarea class="textarea" placeholder="Place some text here" id="turdu" name="turdu" ></textarea>
							</div>
							
							<div class="form-group">
								<label  class="control-label mb-1">Hadees Baab</label>
								<textarea class="textarea" placeholder="Place some text here" id="tbaab" name="tbaab" ></textarea>
							</div>
							<div class="form-group">
								<label  class="control-label mb-1">Hadees Refrence</label>
								 <textarea class="textarea" placeholder="Place some text here" id="trefrence" name="trefrence" ></textarea>
							</div>
							<div>
								<center><input type="submit" class="btn btn-success" name="btn1" value="Update"></center>
							</div>
								<script>CKEDITOR.replace('tenglish'); CKEDITOR.replace('tarabic'); CKEDITOR.replace('turdu'); CKEDITOR.replace('tbaab');CKEDITOR.replace('trefrence');</script>
	...
	$(document).ready(function()
	{
		...
		$.ajax(
		{
			url:"check.php",
			method:"POST",
			data:{check:14,Url:Id},
			success:function(data)
			{
				let jdata=JSON.parse(data);
				$("#tenglish").val(jdata[1]);
				$("#tarabic").val(jdata[2]);
				$("#turdu").val(jdata[3]);
				$("#tbaab").val(jdata[4]);
				$("#trefrence").val(jdata[5]);
				$("#tbname").val(jdata[6]);
				$("#thcname").val(jdata[7]);
			}
		});
		...
		$("#frm").on('submit',function(e)
		{
			...
			$.ajax(
			{
				...
				success:function(data)
				{
					window.location="hadeesview.php";
					$("#show").html(data);
					$("#tenglish").val("");
					$("#tarabic").val("");
					$("#turdu").val("");
